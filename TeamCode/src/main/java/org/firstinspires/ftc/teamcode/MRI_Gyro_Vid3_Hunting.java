...
public class MRI_Gyro_Vid3_Hunting extends OpMode
{
    ...
    public void init() {
        ...
        servo = hardwareMap.servo.get("right_hand"); // left_hand
    ...
    public void loop() {
        ...
        if (zAccumulated > 3 && servoPosition > 0) {  //if gyro is positive, we will decrease servo position but not if servo position is 0
            servoPosition++;
        }

        if (zAccumulated < -3 && servoPosition < 255) {  //if gyro is negative, we will increase servo position but not if servo position is 255
            servoPosition--;
        }
        servo.setPosition((servoPosition / 255));
